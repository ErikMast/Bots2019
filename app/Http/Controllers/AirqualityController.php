...
class AirqualityController extends Controller
{
    ...
    public function magnitudeSeperatorGraph(AirqualityLineChart $request)
    {
        $startDate = $request->get('start_date');
        $endDate = $request->get('end_date');
        $inducedCount = AirqualityEvent::whereDate('date', '>', $startDate)->whereDate('date', '<', $endDate)->where("value", "<", "3")->count();
        $naturalCount = AirqualityEvent::whereDate('date', '>', $startDate)->whereDate('date', '<', $endDate)->where("value", ">=", 3)->count();
        return response()->json([
            $inducedCount, $naturalCount
        ]);
    }
